...
namespace WotCI.API
{
    ...
    )
    {
        ...
        public void SetupCapabilities(PluginTrustLevel trustLevel)
        {
            ...
            _gateway.RegisterCapability(new HealthCapability(_game, trustLevel, auditor));
            _gateway.RegisterCapability(new InventoryCapability(_game, trustLevel, auditor));
            _gateway.RegisterCapability(new CombatCapability(_game, trustLevel, auditor));
            _gateway.RegisterCapability(new GameStateCapability(_game, trustLevel, auditor));

            // Register message bus capability if available
            if (messageBus != null)
            {
                _gateway.RegisterCapability(
                    new MessageBusCapability(messageBus, trustLevel, auditor)
                );
            }

            auditor?.LogCapabilityUsage(
                "api_facade",
                "setup_capabilities",
                [trustLevel.ToString()],
                null,
        ...
        private void AddConvenienceFunctions(Table api, Script script, PluginTrustLevel trustLevel)
        {
            ...
            healthTable["get"] = DynValue.NewCallback(
                (ctx, args) =>
                {
            ...
            {
                healthTable["heal"] = DynValue.NewCallback(
                    (ctx, args) =>
                    {
                        if (args.Count < 1)
                            throw new ArgumentException("heal requires amount parameter");
                ...
                healthTable["set"] = DynValue.NewCallback(
                    (ctx, args) =>
                    {
                        if (args.Count < 1)
                            throw new ArgumentException("set requires health parameter");
            ...
            if (trustLevel >= PluginTrustLevel.Partner)
            {
                goldTable["add"] = DynValue.NewCallback(
                    (ctx, args) =>
                    {
                        if (args.Count < 1)
                            throw new ArgumentException("add requires amount parameter");
                        var amount = (int)args[0].Number;
            ...
            gameTable["getStats"] = DynValue.NewCallback(
                (ctx, args) =>
                {
            ...
            gameTable["getState"] = DynValue.NewCallback(
                (ctx, args) =>
                {
                    var keys =
                        args.Count > 0
            ...
            gameTable["getSnapshot"] = DynValue.NewCallback(
                (ctx, args) =>
                {
        ...
        private void AddMessagingFunctions(
            Table api,
            Script script,
            string scriptId,
        ...
        )
        {
            ...
            msgTable["send"] = DynValue.NewCallback(
                (ctx, args) =>
                {
                    if (args.Count < 2)
                        throw new ArgumentException(
                            "send requires messageType and data parameters"
                        );
                    ...
                    var message = new ScriptMessage
                    {
                        Type = messageType,
                        FromScript = scriptId,
                        Data = data is Dictionary<string, object> dict
                            ? dict
                            : new Dictionary<string, object> { ["value"] = data ?? DBNull.Value },
                    };
            ...
            msgTable["sendTo"] = DynValue.NewCallback(
                (ctx, args) =>
                {
                    if (args.Count < 3)
                        throw new ArgumentException(
                            "sendTo requires targetScript, messageType, and data parameters"
                        );
                    ...
                    var result = _gateway.CallCapability(
                        "message_bus",
                        "send_direct",
                        message,
                        targetScript
            ...
            msgTable["subscribe"] = DynValue.NewCallback(
                (ctx, args) =>
                {
                    if (args.Count < 2)
                        throw new ArgumentException(
                            "subscribe requires messageType and handler parameters"
                        );
                    ...
                    Func<ScriptMessage, Task<ScriptMessage?>> wrappedHandler = msg =>
                    {
                        try
                        {
                            ...
                            if (result.Type == DataType.Table)
                            {
                                ...
                                return Task.FromResult<ScriptMessage?>(
                                    msg.CreateResponse(responseData)
                                );
                            }
        ...
        {
            var utilTable = new Table();
            api["util"] = utilTable;

            // Logging function with rate limiting
            utilTable["log"] = DynValue.NewCallback(
                (ctx, args) =>
                {
                    if (args.Count < 1)
                        return DynValue.Nil;
                    ...
                    // Rate limit logging
                    if (rateLimiter?.IsAllowed("logging", level) == false)
                    {
                        return DynValue.NewBoolean(false);
                    }

                    auditor?.LogCapabilityUsage("logging", level, [message], null, true);
                    rateLimiter?.RecordOperation("logging", level);

                    // In a real implementation, this would integrate with the game's logging system
                    // Only output in non-test environment
                    if (!Environment.GetCommandLineArgs().Any(arg => arg.Contains("test")))
                    {
            ...
            // Safe wait function
            utilTable["wait"] = DynValue.NewCallback(
                (ctx, args) =>
                {
                    if (args.Count < 1)
                        return DynValue.Nil;
            ...
            // Get trust level
            utilTable["getTrustLevel"] = DynValue.NewCallback(
                (ctx, args) =>
                {
            ...
            utilTable["random"] = DynValue.NewCallback(
                (ctx, args) =>
                {
                    if (args.Count == 0)
                    ...
                    if (args.Count == 2)
                        return DynValue.NewNumber(
                            random.Next((int)args[0].Number, (int)args[1].Number)
                        );
    ...
    /// </summary>
    public class MessageBusCapability(
        IScriptMessageBus messageBus,
        PluginTrustLevel trustLevel,
        ISecurityAuditor? auditor = null
    ) : ScriptCapabilityBase("message_bus", auditor)
    {
        ...
        protected override bool IsOperationAllowed(string operation, object[] parameters)
        {
            ...
            {
                "publish" => trustLevel >= PluginTrustLevel.Partner, // Partners can publish
                "send_direct" => trustLevel >= PluginTrustLevel.Partner, // Partners can send direct
        ...
        public override ValidationResult ValidateParameters(string operation, object[] parameters)
        {
            return operation switch
            {
                "publish" when parameters?.Length > 0 && parameters[0] is ScriptMessage =>
                    ValidationResult.Valid(),
                "publish" => ValidationResult.Invalid("Publish requires a ScriptMessage parameter"),
                "send_direct"
                    when parameters?.Length > 1
                        && parameters[0] is ScriptMessage
                        && parameters[1] is string => ValidationResult.Valid(),
                "send_direct" => ValidationResult.Invalid(
                    "Send direct requires ScriptMessage and target script ID"
                ),
                "subscribe" when parameters?.Length > 2 => ValidationResult.Valid(),
                "subscribe" => ValidationResult.Invalid(
                    "Subscribe requires message type, script ID, and handler"
                ),
                "unsubscribe" when parameters?.Length > 1 => ValidationResult.Valid(),
                "unsubscribe" => ValidationResult.Invalid(
                    "Unsubscribe requires message type and script ID"
                ),
        ...
        private object ExecuteSendDirectSync(object[] parameters)
        {
            if (
                parameters?.Length > 1
                && parameters[0] is ScriptMessage message
                && parameters[1] is string targetId
            )
            {
                // Use GetAwaiter().GetResult() which provides better exception handling than .Result
                var response = _messageBus
                    .SendDirectAsync(message, targetId)
                    .GetAwaiter()
                    .GetResult();
                return response;
            }
