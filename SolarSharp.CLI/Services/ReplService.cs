...
namespace SolarSharp.CLI.Services
{
    ...
    public class ReplService : IReplService
    {
        ...
        {
            _logger.LogDebug("Starting REPL with policy: {PolicyName}", options.PolicyName);

            try
            {
                // Create security configuration
                var securityConfig = _securityFactory.Create(
                    options.PolicyName,
                ...
                {
                    _logger.LogDebug(
                        "Applying timeout override: {Timeout}ms",
                        options.TimeoutMs.Value
                    );
                    securityConfig = securityConfig with { TimeoutMs = options.TimeoutMs.Value };
                }

                if (options.MemoryMb.HasValue)
                {
                    _logger.LogDebug(
                        "Applying memory limit override: {Memory}MB",
                        options.MemoryMb.Value
                    );
                    securityConfig = securityConfig with { MaxMemoryMB = options.MemoryMb.Value };
                }
                ...
                await Task.Run(
                    () => RunInterpreterLoop(interpreter, cancellationToken),
                    cancellationToken
                );
            }
        ...
        private void DisplayBanner(ReplOptions options)
        {
            ...
            Console.WriteLine();
            Console.WriteLine($"Policy: {options.PolicyName}");
