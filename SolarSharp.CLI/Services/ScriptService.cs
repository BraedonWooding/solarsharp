...
namespace SolarSharp.CLI.Services
{
    ...
    public class ScriptService : IScriptService
    {
        ...
        /// <summary>
        /// Factory instance responsible for creating security configurations.
        /// Provides the logic to generate security settings based on a specified
        /// security level and optional manifest path.
        /// </summary>
        ...
        /// <param name="securityLevel">Security level for script execution.</param>
            ...
            CancellationToken cancellationToken)
        {
            ...
            _logger.LogDebug("Running script: {ScriptPath} with security level: {SecurityLevel}", 
                scriptPath, securityLevel);
            ...
            try
            {
                ...
                var securityConfig = _securityFactory.Create(securityLevel, manifestPath);
                ...
                var result = await Task.Run(() => ExecuteScript(script, scriptPath), cancellationToken);
        ...
        private void ConfigureScriptGlobals(Script script, string scriptPath, string[] args)
        {
            ...
            if (args.Length > 0)
            {
                script.Globals["..."] = DynValue.NewTuple(args.Select(static a => DynValue.NewString(a)).ToArray());
            }
        ...
        private string FormatScriptError(Script script, ScriptRuntimeException ex)
        {
            ...
            if (ex.CallStack == null || !ex.CallStack.Any()) return message;
            message += "\n\nCall stack:";
