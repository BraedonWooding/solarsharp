...
namespace SolarSharp.CLI.Services
{
    ...
    public class HardwireService : IHardwireService
    {
        ...
        public HardwireService(ILogger<HardwireService> logger)
        {
            ...
            CancellationToken cancellationToken)
        {
            ...
            _logger.LogDebug("Generating {Language} code from {InputPath}", 
                language.ToUpperInvariant(), inputPath);
            ...
            try
            {
                ...
                _logger.LogInformation("Successfully generated {Language} code to {OutputPath}", 
                    language.ToUpperInvariant(), Path.GetFileName(outputPath));
            }
            ...
            bool includeInternals)
        {
            try
            {
                ...
                HardwireCodeGenerationLanguage codeGenLanguage = language.ToLowerInvariant() switch
                {
                    ...
                    _ => throw new ArgumentException($"Unsupported language: {language}. Supported languages are 'cs' and 'vb'.")
                };
                ...
                using (var testStream = new FileStream(inputPath, FileMode.Open, FileAccess.Read))
                {
                    ...
                    if (testStream.Length >= 4)
                    {
                        ...
                        isBytecode = buffer[0] == 0x1B && buffer[1] == 0x4C && 
                                   buffer[2] == 0x75 && buffer[3] == 0x61;
                    }
                }
                ...
                _logger.LogInformation(
                    ...
                    outputSize,
                    File.ReadAllLines(outputPath).Length);
            }
            catch (Exception ex)
            {
