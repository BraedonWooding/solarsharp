    ...
    internal class HardWireCommand : ICommand
    {
        ...
        public static void Generate(string language, string luafile, string destfile, bool allowInternals, string classname, string namespacename)
        {
            ...
            try
            {
                ...
                HardwireGenerator hcg = new(namespacename ?? "HardwiredClasses", classname ?? "HardwireTypes", logger,
                    language == "vb" ? throw new NotImplementedException("We don't support vb hardwiring") : HardwireCodeGenerationLanguage.CSharp)
                {
        ...
        private static string AskQuestion(string prompt, string defval, Func<string, bool> validator, string errormsg)
        {
            while (true)
            {
