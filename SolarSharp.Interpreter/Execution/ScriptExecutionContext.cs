...
public struct ScriptExecutionContext : IScriptPrivateResource
{
    ...
    public DynValue Call(DynValue func, params DynValue[] args)
    {
        ...
        if (func.Type == DataType.ClrFunction)
            while (true)
            {
                ...
                if (ret.Type == DataType.YieldRequest) throw ScriptRuntimeException.CannotYield();
        ...
        while (maxloops > 0)
        {
            ...
            if (v == null && v.IsNil()) throw ScriptRuntimeException.AttemptToCallNonFunc(func.Type);

            func = v;

            if (func.Type == DataType.Function || func.Type == DataType.ClrFunction) return Call(func, args);
        }
    ...
    /// <summary>
    ///     Performs a message decoration before unwinding after an error. To be used in the implementation of xpcall like
    ///     functions.
    /// </summary>
