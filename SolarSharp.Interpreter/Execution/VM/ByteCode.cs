...
internal class ByteCode : RefIdObject
{
    ...
    {
        AppendInstruction(new Instruction(m_CurrentSourceRef)
            { OpCode = OpCode.Debug, Name = str[..Math.Min(32, str.Length)] });
    }
    ...
    {
        return AppendInstruction(new Instruction(m_CurrentSourceRef)
            { OpCode = OpCode.NewTable, NumVal = arraySizeHint, NumVal2 = associativeSizeHint });
    }
    ...
    public Instruction Emit_Swap(int p1, int p2)
    {
        return AppendInstruction(
            new Instruction(m_CurrentSourceRef) { OpCode = OpCode.Swap, NumVal = p1, NumVal2 = p2 });
    }
