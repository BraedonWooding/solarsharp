...
internal sealed partial class Processor
{
        ...
        LuaValue unwindHandler = null)
    {
        ...
        if (fn.Type == DataType.ClrFunction)
        {
            ...
            var ret = fn.Callback.Invoke(new ScriptExecutionContext(this, sref), args, thisCall);
    ...
    private int ExecShortCircuitingOperator(Instruction i, int instructionPtr)
    {
        ...
        if (op.CastToBool() == expectedValToShortCircuit) return i.NumVal;
    ...
    private int ExecIndexSetSingleIdx(Instruction i, int instructionPtr)
    {
        ...
        for (var op = 0; op < 100; op++)
        {
            ...
            if (obj.Type == DataType.Table)
            {
                ...
                newIndexMethod = obj.Table.Set(idx, value, true);
            ...
            else if (obj.Type == DataType.UserData)
            {
                ...
                if (!ud.Descriptor.SetIndex(GetScript(), ud.Object, originalIdx, value, i.OpCode == OpCode.IndexN))
    ...
    private int ExecIndexSetMultiIdx(Instruction i, int instructionPtr)
    {
        ...
        for (var op = 0; op < 100; op++)
        {
            ...
            if (obj.Type == DataType.UserData)
            {
                ...
                if (!ud.Descriptor.SetIndex(GetScript(), ud.Object, originalIdx, value, false))
                    throw ScriptRuntimeException.UserDataMissingField(ud.Descriptor.Name, idx.String);
    ...
    private int ExecIndexGetMultiIdx(Instruction i, int instructionPtr)
    {
        ...
        for (var op = 0; op < 100; op++)
        {
            ...
            if (obj.Type == DataType.UserData)
            {
                ...
                var v = ud.Descriptor.Index(GetScript(), ud.Object, originalIdx, false) ??
                        throw ScriptRuntimeException.UserDataMissingField(ud.Descriptor.Name, idx.String);
                m_ValueStack.Push(v.AsReadOnly());
                return instructionPtr;
            }
