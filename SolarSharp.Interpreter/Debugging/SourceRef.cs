...
public class SourceRef
{
    ...
    internal int GetLocationDistance(int sourceIdx, int line, int col)
    {
        ...
        if (line > FromLine && line < ToLine) return 0;

        if (line < FromLine) return (FromLine - line) * PER_LINE_FACTOR;
    ...
    public string FormatLocation(Script script, bool forceClassicFormat = false)
    {
        ...
        if (FromLine == ToLine)
        {
            if (FromChar == ToChar) return string.Format("{0}:({1},{2})", sc.Name, FromLine, FromChar, ToLine, ToChar);
