...
public abstract class DispatchingUserDataDescriptor : IUserDataDescriptor, IOptimizableDescriptor
{
    ...
    {
        foreach (var m in m_MetaMembers.Values.OfType<IOptimizableDescriptor>())
            m.Optimize();

        foreach (var m in m_Members.Values.OfType<IOptimizableDescriptor>())
            m.Optimize();
    }
    ...
    public virtual DynValue Index(Script script, object obj, DynValue index, bool isDirectIndexing)
    {
        ...
        if (v == null && (Script.GlobalOptions.FuzzySymbolMatching & FuzzySymbolMatchingBehavior.UpperFirstLetter) ==
        ...
        if (v == null && (Script.GlobalOptions.FuzzySymbolMatching & FuzzySymbolMatchingBehavior.PascalCase) ==
            FuzzySymbolMatchingBehavior.PascalCase) v = TryIndex(script, obj, UpperFirstLetter(Camelify(index.String)));
    ...
    ///     __tobool is dispatched to an implicit or explicit conversion operator to bool. If that fails, operator true is
    ///     used.
    ///     <param name="script">The script originating the request</param>
    ///     <param name="obj">The object (null if a static request is done)</param>
