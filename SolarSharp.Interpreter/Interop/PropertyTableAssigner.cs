...
public class PropertyTableAssigner<T> : IPropertyTableAssigner
{
    ...
    /// </summary>
    /// <param name="expectedMissingProperties">
    ...
    /// <summary>
    ///     Adds an expected missing property, that is an expected field in the table with no corresponding property in the
    ///     object.
    /// </summary>
    ...
    ///     A field does not correspond to any property and that property is not one of
    ///     the expected missing ones.
    /// </exception>
...
public class PropertyTableAssigner : IPropertyTableAssigner
{
    ...
    public PropertyTableAssigner(Type type, params string[] expectedMissingProperties)
    {
        ...
        foreach (var property in expectedMissingProperties) m_PropertyMap.Add(property, null);

        foreach (var pi in Framework.Do.GetProperties(m_Type))
        foreach (var attr in pi.GetCustomAttributes(true).OfType<SolarSharpPropertyAttribute>())
        {
