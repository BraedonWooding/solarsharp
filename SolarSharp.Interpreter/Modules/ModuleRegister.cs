...
namespace SolarSharp.Interpreter.Modules
{
    ...
    public static class ModuleRegister
    {
        ...
        public static Table RegisterModuleType(this Table gtable, Type t)
        {
            ...
            foreach (FieldInfo fi in Framework.Do.GetFields(t).Where(_mi => _mi.IsStatic && _mi.GetCustomAttributes(typeof(SolarSharpModuleMethodAttribute), false).ToArray().Length > 0))
            {
                SolarSharpModuleMethodAttribute attr = (SolarSharpModuleMethodAttribute)fi.GetCustomAttributes(typeof(SolarSharpModuleMethodAttribute), false).First();
