...
public static class ModuleRegister
{
    ...
    public static Table RegisterModuleType(this Table gtable, Type t)
    {
        ...
        foreach (var mi in Framework.Do.GetMethods(t).Where(__mi => __mi.IsStatic))
            if (mi.GetCustomAttributes(typeof(SolarSharpModuleMethodAttribute), false).Length > 0)
            {
                var attr = (SolarSharpModuleMethodAttribute)mi
 ...
 mi.CreateDelegate(typeof(Func<ScriptExecutionContext, CallbackArguments, DynValue>));
        ...
        foreach (var fi in Framework.Do.GetFields(t).Where(_mi =>
                     _mi.IsStatic && _mi.GetCustomAttributes(typeof(SolarSharpModuleMethodAttribute), false).Length >
                     0))
        {
            var attr = (SolarSharpModuleMethodAttribute)fi
                .GetCustomAttributes(typeof(SolarSharpModuleMethodAttribute), false).First();
        ...
        foreach (var fi in Framework.Do.GetFields(t).Where(_mi =>
                     _mi.IsStatic && _mi.GetCustomAttributes(typeof(SolarSharpModuleConstantAttribute), false).Length >
                     0))
        {
            var attr = (SolarSharpModuleConstantAttribute)fi
    ...
    {
        var attr = (SolarSharpModuleAttribute)Framework.Do
            .GetCustomAttributes(t, typeof(SolarSharpModuleAttribute), false).First();
