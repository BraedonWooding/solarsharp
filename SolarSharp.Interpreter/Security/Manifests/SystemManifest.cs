...
namespace SolarSharp.Interpreter.Security.Manifests
{
    ...
    public sealed class SystemManifest : Manifest
    {
        ...
        public static SystemManifest FromManifest(Manifest manifest)
        {
            ...
            var systemManifest = new SystemManifest(validateComplete: true)
            {
                ...
                Files = new Dictionary<string, ManifestFileEntry>(manifest.Files),
                Includes = new List<string>(manifest.Includes),
        ...
        private static ManifestPolicy CopyPolicy(ManifestPolicy source)
        {
            return new ManifestPolicy
            {
                ...
                PreventRunString = source.PreventRunString,
                PreventInternalDynamicCode = source.PreventInternalDynamicCode,
        ...
        private static SystemManifest CreateDesktop()
        {
            return CreateSystemManifest("Desktop - PHP-like environment for desktop scripts")
                .WithPolicy(policy => policy
                    ...
                    .WithFileAccess(FilePermissions.SandboxedReadWrite, DirectoryPermissions.ListAndCreateFiles)
                    .WithDesktopModules()
        ...
        private static SystemManifest CreateJailed()
        {
            return CreateSystemManifest("Jailed - Maximum security sandbox")
                .WithPolicy(policy => policy
                    ...
                    .WithFileAccess(FilePermissions.None, DirectoryPermissions.None) // No file access at all
    ...
    internal class PolicyBuilder
    {
        ...
        {
            _policy.TimeoutMs = 0; // 0 means no timeout limit
            _policy.MaxMemoryMB = 0; // 0 means no memory limit
            _policy.MaxInstructions = 0; // 0 means no instruction limit
            return this;
        }
        ...
        {
            _policy.DefaultFileAccess = FilePermissions.ReadWrite.ToManifestString();
            _policy.DefaultDirectoryAccess = DirectoryPermissions.ListAndCreateFiles.ToManifestString();
        ...
        {
            _policy.DefaultFileAccess = FilePermissions.None.ToManifestString();
            _policy.DefaultDirectoryAccess = DirectoryPermissions.None.ToManifestString();
            _policy.AllowNetworkAccess = false;
            _policy.AllowEnvironmentAccess = false;
            return this;
        }

        public PolicyBuilder WithFileAccess(FilePermissions fileAccess = FilePermissions.SandboxedReadWrite, DirectoryPermissions directoryPermissions = DirectoryPermissions.ListAndCreateFiles)
        {
            _policy.DefaultFileAccess = fileAccess.ToManifestString();
            _policy.DefaultDirectoryAccess = directoryPermissions.ToManifestString();
            return this;
        }
        ...
        public PolicyBuilder EnableSandbox(bool antiPolymorphism = true)
        {
            ...
            if (antiPolymorphism)
            {
                ...
                _policy.PreventRunString = true;
                _policy.PreventInternalDynamicCode = true;
            }
            return this;
        }
    ...
    internal static class ManifestPolicyExtensions
    {
        internal static ManifestPolicy ToManifestPolicy(this SecurityConfiguration config)
        {
            return new ManifestPolicy
            {
                ...
                DefaultFileAccess = config.FileSystem.DefaultFilePermissions.ToManifestString(),
                DefaultDirectoryAccess = config.FileSystem.DefaultDirectoryPermissions.ToManifestString(),
                ...
                PreventRunString = config.AntiPolymorphism.PreventRunString,
                PreventInternalDynamicCode = config.AntiPolymorphism.PreventInternalDynamicCode,
        ...
        private static bool IsEnabled(this AntiPolymorphismPolicy policy)
        {
            return policy.AllowOnlyLuaExtension || 
                   ...
                   policy.PreventRunString ||
                   policy.PreventInternalDynamicCode || 
