    ...
    public static class ManifestAutoLoader
    {
        ...
        private static LoadedManifest LoadAndVerifyManifest(string path, HashSet<string> loadingPaths, HashSet<string> loadedManifests, int depth)
        {
            ...
            try
            {
                ...
                if (manifest.Security?.Signature != null && manifest.Security?.PublicKey != null)
                {
            ...
            try
            {
                ...
                switch (publicKey)
                {
                    case RSA rsa:
                        ...
                        return rsa.VerifyData(manifestBytes, signatureBytes, 
                    ...
                    case ECDsa ecdsa:
                        ...
                        return ecdsa.VerifyData(manifestBytes, signatureBytes, 
        ...
        public static SecurityConfigurationOverrides CreateOverridesFromManifest(string scriptPath)
        {
            ...
            {
                ValidateUntrustedOverrides(overrides);
            }
        ...
        /// </summary>
        private static void ValidateUntrustedOverrides(SecurityConfigurationOverrides overrides)
        {
            // Untrusted manifests can only make things more restrictive
            // This validation ensures they can't grant additional permissions

            // Timeout can only be decreased (or set to non-zero if it was zero)
            if (overrides.TimeoutMs.HasValue && overrides.TimeoutMs.Value == 0)
            {
                throw new ManifestFormatException("Untrusted manifests cannot disable timeout limits", "ValidateUntrustedOverrides");
            }

            // Memory limits can only be decreased (or set to non-zero if it was zero)
            if (overrides.MaxMemoryMB.HasValue && overrides.MaxMemoryMB.Value == 0)
            {
                throw new ManifestFormatException("Untrusted manifests cannot disable memory limits", "ValidateUntrustedOverrides");
            }

            // Instruction limits can only be decreased (or set to non-zero if it was zero)  
            if (overrides.MaxInstructions.HasValue && overrides.MaxInstructions.Value == 0)
            {
                throw new ManifestFormatException("Untrusted manifests cannot disable instruction limits", "ValidateUntrustedOverrides");
            }

            // File access can only be more restrictive
            if (overrides.DefaultFileAccess.HasValue && overrides.DefaultFileAccess.Value == FileAccess.ReadWrite)
            {
            ...
            if (overrides.PreventDynamicCode.HasValue && !overrides.PreventDynamicCode.Value)
            {
                throw new ManifestFormatException("Untrusted manifests cannot allow dynamic code execution", "ValidateUntrustedOverrides");
            }
        }
