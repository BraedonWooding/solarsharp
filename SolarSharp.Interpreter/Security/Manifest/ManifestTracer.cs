    ...
    public class ManifestTracer : ISecurityEventHandler
    {
        ...
        public Manifest GenerateManifest()
        {
            ...
            if (_traceData.MaxExecutionTime > 0)
            {
                ...
                builder.WithTimeout(Math.Max(timeout, 5)); // Minimum 5 seconds
            }
            ...
            if (_traceData.MaxMemoryUsed > 0)
            {
                ...
                builder.WithMemoryLimit(Math.Max(memoryMB, 10)); // Minimum 10MB
            }
        ...
        private void AddFileAccessRules(ManifestBuilder builder)
        {
            ...
            foreach (var pattern in patterns)
            {
                ...
                {
                    builder.AddFileRule(pattern.Key, FileAccess.ReadWrite);
                }
                else if (pattern.Value.HasFlag(FileAccessType.Read))
                {
                    builder.AddFileRule(pattern.Key, FileAccess.Read);
                }
            }
            ...
            foreach (var pattern in dirPatterns)
            {
                if (pattern.Value.HasFlag(FileAccessType.Write))
                {
                    builder.AddDirectoryRule(pattern.Key, DirectoryAccess.ListAndCreateFiles);
                }
                else if (pattern.Value.HasFlag(FileAccessType.Read))
                {
                    builder.AddDirectoryRule(pattern.Key, DirectoryAccess.List);
                }
            }
        }
