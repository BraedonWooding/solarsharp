...
internal class BinaryOperatorExpression : Expression
{
    ...
    private bool EvalComparison(LuaValue l, LuaValue r, Operator op)
    {
        ...
        {
            case Operator.Less:
                if (l.Type == DataType.Number && r.Type == DataType.Number) return l.Number < r.Number;

                if (l.Type == DataType.String && r.Type == DataType.String) return l.String.CompareTo(r.String) < 0;
            ...
            case Operator.LessOrEqual:
                ...
                throw new DynamicExpressionException("Attempt to compare non-numbers, non-strings.");
            case Operator.Equal:
                if (ReferenceEquals(r, l)) return true;
