...
public class ScriptRuntimeException : InterpreterException
{
    ...
    ///     while <see cref="BadArgumentNoValue" /> creates a message like "bad argument #xxx to 'yyy' (zzz expected, got no
    ///     value)"
    /// </summary>
    /// <param name="argNum">The argument number (0-based).</param>
    /// <param name="funcName">Name of the function generating this error.</param>
    ...
    {
        return new ScriptRuntimeException("cannot convert a {0} to a clr type {1}", t.ToString().ToLowerInvariant(),
            t2.FullName);
    }
    ...
    {
        return new ScriptRuntimeException("cannot find a conversion from a SolarSharp {0} to a clr {1}",
    ...
    {
        return new ScriptRuntimeException("attempt to access instance member {0}.{1} from a static userdata",
            typeDescr.Name, desc.Name);
    }
