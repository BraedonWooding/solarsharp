...
public class LoadModule
{
        ...
        Table defaultEnv)
    {
        ...
        {
            var S = executionContext.GetScript();
            var filename = args.AsType(0, "loadfile", DataType.String);
            var env = args.AsType(2, "loadfile", DataType.Table, true);

            var fn = S.LoadFile(filename.String, env.IsNil() ? defaultEnv : env.Table);
