...
namespace SolarSharp.Hardwire
{
    ...
    public sealed class HardwireCodeGenerationContext
    {
        ...
        private CodeStatementCollection m_InitStatements;
        private CodeTypeDeclaration m_KickstarterClass;
        private readonly CodeNamespace m_Namespace;
        private readonly ICodeGenerationLogger m_Logger;
        private readonly Stack<string> m_NestStack = new Stack<string>();
        ...
        )
        {
            ...
            Comment("----------------------------------------------------------");
            Comment(
                "Generated by SolarSharp.Hardwire v.{0}",
                Assembly.GetExecutingAssembly().GetName().Version
            );
            Comment("Compatible with MoonSharp v.{0} or equivalent", Script.VERSION);
            Comment("----------------------------------------------------------");

            var extraComments = language.GetInitialComment();

            if (extraComments != null)
            {
                foreach (var str in extraComments)
                    Comment("{0}", str);
        ...
        internal void GenerateCode(Table table)
        {
            ...
            {
                m_Logger.LogError($"Internal error, code generation aborted : {ex}");
            }
        }
        ...
        /// <param name="members">The members.</param>
        /// <returns></returns>
        /// <exception cref="ArgumentException">table cannot be dispatched as it has no class or class of invalid type.</exception>
        ...
        private void GenerateKickstarter(string className)
        {
            ...
            var ctor = new CodeConstructor { Attributes = MemberAttributes.Private };
            cl.Members.Add(ctor);
            ...
            m_InitStatements = m.Statements;
            m_KickstarterClass = cl;
        }
    }
}
